...
function Set-MfaState 
{
   <#
         .SYNOPSIS
         Describe purpose of "Set-MfaState" in 1-2 sentences.

         .DESCRIPTION
         Add a more complete description of what the function does.

         .PARAMETER ObjectId
         Describe parameter -ObjectId.

         .PARAMETER UserPrincipalName
         Describe parameter -UserPrincipalName.

         .PARAMETER State
         Describe parameter -State.

         .EXAMPLE
         Set-MfaState -ObjectId Value -UserPrincipalName Value -State Value
   ...
   Process {
      Write-Verbose -Message ("Setting MFA state for user '{0}' to '{1}'." -f $ObjectId, $State)
