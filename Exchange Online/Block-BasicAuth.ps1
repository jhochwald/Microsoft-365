...
{
   Write-Host
   Write-Host -ForegroundColor $AssessmentColor -Object 'Modern Authentication for Exchange online is not enabled'
   Write-Host 
   ...
   {
      Write-Host
      Write-Host -ForegroundColor $AssessmentColor -Object 'Modern Authentication will not be enabled'
   }
}
...
$AuthAnswer = Read-Host -Prompt "Do you want to make 'Block Basic Auth' the default authentication policy for the organization? WARNING: This action will prevent older clients from connecting to Exchange Online. Type Y or N and press Enter to continue"
...
{
   Write-Host 
   Write-Host -ForegroundColor $AssessmentColor -Object "Block Basic Auth will not be set as the default policy. Please assign this policy to users individually using: Set-User -Identity <username> -AuthenticationPolicy 'Block Basic Auth' "
   Write-Host 
}
