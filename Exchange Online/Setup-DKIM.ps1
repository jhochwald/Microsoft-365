...
Write-Host -ForegroundColor $MessageColor -Object 'Enter the (Points to) CNAME values in DNS for selector1._domainkey and selector2._domainkey:'
Write-Host 

Get-DkimSigningConfig $DomainName | Select-Object -Property Domain, *cname 
...
if (((get-dkimsigningconfig -identity $DomainName -ErrorAction silent).enabled) -eq $False) 
{
   set-dkimsigningconfig -identity $DomainName -enabled $true
}
#If it doesn't exist - create new config
if (-not (get-dkimsigningconfig -identity $DomainName -erroraction silent)) 
{
