...
$Answer = Read-Host -Prompt 'Do you want to modify the default mobile device mailbox policy to require 4-digit passcode with encryption? Type Y or N and press Enter to continue'
...
if ($Answer -eq 'y' -or $Answer -eq 'yes') 
{
   ...
   Write-Host 
   Write-Host -ForegroundColor $MessageColor -Object 'The default mobile device mailbox policy has been modified'
}
else 
{
   Write-Host 
   Write-Host -ForegroundColor $AssessmentColor -Object 'Default mobile device mailbox policy will not be modified; consider using Intune to setup MDM or MAM instead'
}
